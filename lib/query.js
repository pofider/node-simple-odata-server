...
let transformResultToAtom = (cfg, collection, result, res) => {
  ...
  if(model) {
    ...
    if(entityNamespacedType) {
      ...
      console.log(`justTheType: `, justTheType)
  ...
  console.log(`entityNamespacedType: `, entityNamespacedType)
    ...
    for(let i = 0; i < numValueItems; i++) {      
      ...
      for(let j = 0; j < numKeysInValue; j++) {
        ...
        if(typeForField === 'Edm.DateTime') {
          res.write(`<d:${field} m:type="${typeForField}">2017-06-08T22:59:59.999Z</d:${field}>`)          
        } else {
